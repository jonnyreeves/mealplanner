...
class MealPlannerDb {
  ...
  getPlanByRange(startIsoDate, endIsoDate) {
    ...
    let numRows;
    let startRowIdx;
    let queryNextWeek = endDate.getTime() >= nextRolloverDate.getTime();
    ...
    queryPlan.forEach(query => {
      let targetSheet = (query.sheet === "this-week") ? this.sheets.thisWeek() : this.sheets.nextWeek();
  ...
  _initMealCache() {
    ...
    for (var i = 0; i < values.length; i++) {
      thisRow = values[i];
      let mealName = String(thisRow[0]).toLowerCase();
  ...
  getNextRolloverDate(startDate) {
    var res = new Date(startDate.getTime());
