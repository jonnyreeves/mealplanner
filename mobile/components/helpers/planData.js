...
export function toPlannerGridData(planEntries) {
    ...
    (planEntries).forEach(item => {      
        ...
        allGridData.push({ id: `${item.date}-lunch`, name: item.lunch.name, slot: 'lunch', date: item.date });
        allGridData.push({ id: `${item.date}-dinner`, name: item.dinner.name, slot: 'dinner', date: item.date });
    });
